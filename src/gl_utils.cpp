...
void redraw_image_3d(GLFWwindow *window, Shader &shader, unsigned int texture,
  ...
  glDrawElements(GL_TRIANGLES, 3 * 2 * (state->height - 1) * (state->width - 1),
                 GL_UNSIGNED_INT, 0);
...
void compute_julia_sp(const ProgramState &state,
  ...
  CUDA_CHECK(cudaGraphicsResourceGetMappedPointer((void **)&d_color_buffer,
                                                  nullptr, cudaPboColor));

  float *d_smoothed_buffer = nullptr;
  CUDA_CHECK(cudaGraphicsMapResources(1, &cudaPboSmoothed, 0));
  CUDA_CHECK(cudaGraphicsResourceGetMappedPointer((void **)&d_smoothed_buffer,
                                                  nullptr, cudaPboSmoothed));

  float *d_vbo = nullptr;
  CUDA_CHECK(cudaGraphicsMapResources(1, &cudaVbo, 0));
  CUDA_CHECK(
      cudaGraphicsResourceGetMappedPointer((void **)&d_vbo, nullptr, cudaVbo));
  ...
  nppiFilterGaussBorder_32f_C1R_Ctx(
      d_color_buffer, sizeof(float) * state.width, size, NppiPoint{0, 0},
      d_smoothed_buffer, sizeof(float) * state.width, size, NPP_MASK_SIZE_9_X_9,
