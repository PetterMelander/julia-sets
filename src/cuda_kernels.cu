...
__device__ uchar3 map_color(float intensity) {
    ...
    if (intensity < MAX_ITERS) {
      rgb.x = (unsigned char)(__sinf(intensity * 0.05f + 0.5f) * 127 + 128);
      rgb.y = (unsigned char)(__sinf(intensity * 0.05f + 1.0f) * 127 + 128);
      rgb.z = (unsigned char)(__sinf(intensity * 0.05f + 2.0f) * 127 + 128);
    }
    return rgb;
}
